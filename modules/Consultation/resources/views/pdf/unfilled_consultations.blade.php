...
<body>
    <div class="header">
        ...
        <div class="subtitle">
            Typ: {{ $consultationType === \Modules\Consultation\Domain\Enums\ConsultationType::Semester ? __('consultation::consultation.Semester Consultations') : __('consultation::consultation.Session Consultations') }}
        </div>
    </div>
        ...
        <table>
            ...
            <tbody>
                @foreach($unfilledWorkers as $index => $worker)
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $worker->reportIdentityLabel() }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
