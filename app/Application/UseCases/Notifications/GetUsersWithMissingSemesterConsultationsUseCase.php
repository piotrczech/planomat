...
final readonly class GetUsersWithMissingSemesterConsultationsUseCase
{
    ...
    public function execute(): Collection
    {
        ...
        if (!$settings->get('notifications_semester_consultations_enabled')) {
            return collect();
        }
        ...
        $daysAfter = (int) $settings->get('notifications_semester_consultations_days_after', 21);
        ...
        foreach ($scientificWorkers as $user) {
            $hasConsultations = SemesterConsultation::where('scientific_worker_id', $user->id)
