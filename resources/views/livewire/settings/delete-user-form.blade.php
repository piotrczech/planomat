...
<section class="mt-10 space-y-6">
    <div class="relative mb-5">
        <flux:heading>{{ __('app.Delete account') }}</flux:heading>
        <flux:subheading>{{ __('app.Delete your account and all of its resources') }}</flux:subheading>
    </div>
    ...
    <flux:modal name="confirm-user-deletion" :show="$errors->isNotEmpty()" focusable class="max-w-lg">
        <form wire:submit="deleteUser" class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('app.Are you sure you want to delete your account?') }}</flux:heading>
            ...
            <div class="flex justify-end space-x-2 rtl:space-x-reverse">
                ...
                <flux:button variant="danger" type="submit">{{ __('app.Delete account') }}</flux:button>
            </div>
        </form>
    </flux:modal>
</section>
