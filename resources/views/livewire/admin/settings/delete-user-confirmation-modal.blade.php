<div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="archive-user-modal-title" role="dialog" aria-modal="true" x-data="{ show: @entangle('isVisible').live }" x-show="show" x-cloak>
        ...
        <div class="relative inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl dark:bg-neutral-900 sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6"
            ...
            <div class="sm:flex sm:items-start">
                ...
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <h3 class="text-lg font-medium leading-6 text-neutral-900 dark:text-neutral-100" id="archive-user-modal-title">
                        {{ __('admin_settings.users.modal_archive.Confirm Archive Title') }}
                    </h3>
                    <div class="mt-2">
                        <p class="text-sm text-neutral-500 dark:text-neutral-400">
                            {{ __('admin_settings.users.modal_archive.Are you sure') }}
                            @if ($userToArchive)
                                <strong class="font-semibold text-neutral-700 dark:text-neutral-200">{{ $userToArchive->fullName() }} ({{ $userToArchive->display_email }})</strong>?
                            @else
                                {{ __('admin_settings.users.modal_archive.this user') }}?
                            @endif
                            {{ __('admin_settings.users.modal_archive.This action can be reverted for 24 hours') }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-5 sm:mt-6 sm:flex sm:flex-row-reverse">
                <button wire:click="confirmArchive" type="button" wire:loading.attr="disabled"
                    ...
                    {{ __('admin_settings.users.modal_archive.Archive User') }}
                </button>
                <button wire:click="cancel" type="button" wire:loading.attr="disabled"
                    ...
                    {{ __('admin_settings.users.modal_archive.Cancel') }}
                </button>
            </div>
        </div>
    </div>
